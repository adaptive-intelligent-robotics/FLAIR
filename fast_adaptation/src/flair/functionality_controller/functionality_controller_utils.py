...
def compute_borders(all_descriptors: jnp.ndarray) -> jnp.ndarray:
    ...
    border_idx = jax.vmap(lambda x : jnp.any(jnp.isin(jnp.abs(x), max_value)))(all_descriptors)
    return border_idx

def compute_clipping(border_idx: jnp.ndarray, all_corrected_descriptors: jnp.ndarray) -> Tuple[float, float]:
...
def compute_metric_map(all_corrected_descriptors: jnp.ndarray, uncertainties: jnp.ndarray):
