...
def start_rl_training_process(
    ...
    while True:
        ...
        if DEBUG_RL_TRAINING:
            ...
            try:
                writeEntry(influx_client, "/lqr_debug_timing", fields)
            except BaseException as e:
                logger.warning("Warning: Influx sending failed.")
                logger.warning(e)
...
class Adaptation(Node):
    def __init__(
        ...
        from functionality_controller.functionality_controller_lqr import (
            FunctionalityControllerLQR,
        )
        ...
        # Init Adaptation
        self._adaptation = FunctionalityControllerLQR(
            min_command=MIN_COMMAND, 
            max_command=MAX_COMMAND,
        )
        self.state = None
