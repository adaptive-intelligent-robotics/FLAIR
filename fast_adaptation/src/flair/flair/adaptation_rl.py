...
# Import the config
from src.flair.flair.adaptation_config_rl import *
...
class SensorCollection(Node):
    ...
    def _send_sensor(self) -> None:
        ...
        if not self.start_collection:
            try:
                self.rl_collection_to_sensor_collection_start_queue.get_nowait()
                self.start_collection = True
                self._logger.debug(
                    "Got signal from RLCollection, starting Sensor Collection."
                )
            except BaseException:
                pass
...
def start_rl_collection_process(
    ...
    while True:
        ...
        if next_command_x is not None:
            ...
            try:
                writeEntry(
                    influx_client, "/rl_datapoint_filter", [("filter_code", error_code)]
                )
            except BaseException as e:
                logger.warning("Warning: Influx sending failed.")
                logger.warning(e)
...
def start_rl_training_process(
    ...
    while True:
        ...
        if DEBUG_RL_TRAINING:
            ...
            try:
                writeEntry(influx_client, "/rl_debug_timing", fields)
            except BaseException as e:
                logger.warning("Warning: Influx sending failed.")
                logger.warning(e)
...
class Adaptation(Node):
    def __init__(
        ...
        from functionality_controller.functionality_controller_rl import (
            FunctionalityControllerRL,
        )
        ...
        self.rl_training_to_adaptation_model_queue = (
            rl_training_to_adaptation_model_queue
        )
        self.sensor_collection_to_adaptation_datapoint_queue = (
            sensor_collection_to_adaptation_datapoint_queue
        )
        self.rl_training_to_adaptation_start_queue = (
        ...
        # Init Adaptation
        self._adaptation = FunctionalityControllerRL(
            min_command=MIN_COMMAND, 
            max_command=MAX_COMMAND,
            learning_rate=LEARNING_RATE,
        )
    ...
    def _cmd_vel_human_callback(self, msg: HumanControl) -> None:
        ...
        # Avoid passing commands while the RLTraining is starting
        if not self.start_adaptation:
            try:
                self.rl_training_to_adaptation_start_queue.get_nowait()
    ...
    def adaptation(self, msg: HumanControl) -> FunctionalityControllerControl:
        ...
        try:
            (
                params,
            ) = self.rl_training_to_adaptation_model_queue.get_nowait()

            self._adaptation.params = params
        ...
        # If the Adaptation is on, apply the Adaptation correction
            ...
            command_x = rl_prediction_x
            command_y = rl_prediction_y
            command_flipper = rl_prediction_flipper

            new_msg.joystick.linear.x = command_x
            new_msg.joystick.angular.z = command_y
            new_msg.flipper.angular.x = command_flipper

        #self._logger.debug(f"{rl_prediction_x - cmd_lin_x}, {rl_prediction_y - cmd_ang_z}")
        ...
        self.adaptation_to_rl_collection_command_queue.put(
            (
                deepcopy(command_x),
                deepcopy(command_y),
                deepcopy(rl_prediction_x),
                deepcopy(rl_prediction_y),
...
def start_adaptation_process(
    ...
    rclpy.init(args=args)
    antoine = Adaptation(
        rl_training_to_adaptation_model_queue=rl_training_to_adaptation_model_queue,
        ...
        rl_training_to_adaptation_start_queue=rl_training_to_adaptation_start_queue,
        loglevel=loglevel,
    )
    rclpy.spin(antoine)
...
def main():
    ...
    rl_training_to_adaptation_start_queue = context.Queue()

    rl_training_to_adaptation_model_queue = context.Queue()
